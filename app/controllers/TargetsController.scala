...
object TargetsController extends Controller {
  ...
  val targetForm: Form[Target] = Form(
    ...
    mapping(
      ...
      "title" -> text,
      "numberOfPositions" -> optional(number),
      ...
      target => {
        ...
        request.body.file("image") match {
          case Some(image) =>
            ...
            Target.save(Target(id, title = target.title, numberOfPositions = target.numberOfPositions, image_id=image_id, updated = Option(new Date())))
            Home.flashing(
          ...
          case None => {
            Target.save(Target(id, title = target.title, numberOfPositions=None, image_id=None, updated = Option(new Date())))
